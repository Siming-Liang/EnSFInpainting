...
# define likelihood score
    ...
    def state_update_normalized(self,x_input,state_target_input, obs_input,sparse_idx,current,arcindex):
        ...
        temp_obs_sigma[arc_index] =torch.max(torch.abs((torch.tan(torch.atan(mean_x_state_obs[arc_index])-0.01) - mean_x_state_obs[arc_index])/mean_x_state_obs[arc_index]) / std_x_state[arc_index] , torch.tensor(0.0001, device = self.device))
        #print(temp_obs_sigma.min(),'max1',temp_obs_sigma.max())
        torch.cuda.empty_cache()
